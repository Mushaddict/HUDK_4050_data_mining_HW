...
*`备注：df数据集，以下这些数据的生成在之前的代码中，后续统一归入这部分`*
...
类别可以删除，连续变量"total_Scholarship", "total_Work_Study" 或许先不删除，转换成类别变量？？
...
Firstly, we conducted a correlation test （这部分其实有点问题，ggcorr只能做`"pearson"` `"kendall"` or `"spearman"`）的相关性，但是均不适用于二分变量的相关行检验。
